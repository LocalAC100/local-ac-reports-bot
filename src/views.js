...
<head>
  ...
  <title>${escape(title)} Â· Control Room</title>
...
<button class="mobile-nav-toggle" onclick="document.querySelector('.primary-nav').classList.toggle('open')" aria-label="Open menu">â°</button>`;
}
...
<div class="kpi-grid">
  <div class="kpi-card">
    ...
    <div class="kpi-sub">${activeNow.map(escape).join(" Â· ") || "â"}</div>
  </div>
  <div class="kpi-card">
...
<section class="panel">
  ...
  ${discrepancies.length === 0
    ? `<div class="empty-good">â Everyone scheduled today is clocked in. No issues.</div>`
    : `<ul class="alert-list">${discrepancies.map(d => `<li><strong>${escape(d.employee)}</strong> â ${escape(d.detail)}</li>`).join("")}</ul>`}
</section>
...
<section class="panel">
  ...
  ${recentAlerts.length === 0
    ...
    : `<ul class="alert-list">${recentAlerts.map(a => `<li><span class="badge badge-red">ð´ ${a.minutes_elapsed}m</span> <strong>${escape(a.contact_name || "(unnamed)")}</strong> Â· ${escape(a.phone || "")} Â· ${escape(a.fired_at)}</li>`).join("")}</ul>`}
</section>`,
  });
}
...
<div class="page-head">
  <h1>Ask Claude</h1>
  <span class="page-sub">Ask anything about your business â Claude has access to Hubstaff, GoHighLevel, and Jobber.</span>
</div>
${!hasApiKey ? `<div class="banner-warn">â  Anthropic API key not configured yet. Add <code>ANTHROPIC_API_KEY</code> in Render's Environment tab to activate.</div>` : ""}
<div class="chat-wrap">
...
form?.addEventListener('submit', async (e) => {
  ...
  history.insertAdjacentHTML('beforeend', '<div class="msg msg-assistant pending"><div class="msg-role">Claude</div><div class="msg-body">â¦thinkingâ¦</div></div>');
...
<section class="panel">
  <table class="data-table">
    ...
    <tbody>${users.map(u => `<tr>
      ...
      <td class="muted">${escape((u.last_login_at || "â").slice(0, 16))}</td>
    </tr>`).join("")}</tbody>
  </table>
</section>
<section class="panel">
...
export function grossProfitPage({ user, jobs, unmatched, inventory, status, flash, filter = {}, totalCount = 0, totalPaid = 0, grandTotalCount = 0 }) {
...
<section class="panel" style="background:#f9fafc">
  ...
  const rows = jobs.map((j) => {
    ...
    return `<tr>
      ...
      <td>${escape(j.jobber_invoice_number || "â")}</td>
  ...
  const tableHtml = empty
    ? `<div class="empty-good">No jobs yet â waiting for first Jobber invoice. Open the <a href="/gross-profit/setup">setup checklist</a> to wire up the integrations.</div>`
    : `<table class="data-table">
        <thead><tr>
  ...
  const unmatchedHtml = unmatched.length === 0 ? "" : `
    <section class="panel panel-warn">
      <h2>â  Unmatched Invoices â Review Manually</h2>
      <table class="data-table">
        ...
        <tbody>${unmatched.map(u => `<tr>
          ...
          <td>${escape(u.po_name || "â")}</td>
          <td>${money(u.total_amount)}</td>
          <td>${u.att_id ? `<a href="/gross-profit/attachment/${u.att_id}">${escape(u.filename || "PDF")}</a>` : "â"}</td>
          <td class="muted">${escape((u.created_at || "").slice(0,16))}</td>
          <td><a href="/gross-profit/unmatched/${u.id}/resolve">Resolve â</a></td>
        </tr>`).join("")}</tbody>
      </table>
    </section>`;
  ...
  // Status panel â shows whether each connector is wired up
  const statusItem = (label, ok, hint) =>
    `<li><span class="status-dot ${ok ? "ok" : "off"}"></span> <strong>${escape(label)}</strong> â ${ok ? "ready" : escape(hint || "not configured")}</li>`;
  ...
  const setupHtml = `
    <section class="panel">
      ...
      <ul class="status-list">
        ${statusItem("Jobber sync (invoices â rows)", status.jobber, "Set JOBBER_CLIENT_ID, CLIENT_SECRET, ACCESS_TOKEN, REFRESH_TOKEN")}
        ${statusItem("Google Sheets (Chris's sheet â rows)", status.sheets && status.chrisSheetId, "Set GOOGLE_SA_JSON + CHRIS_SHEET_ID")}
        ${statusItem("Mirror sheet (rows â Google Sheet)", status.sheets && status.mirrorSheetId, "Set GOOGLE_SA_JSON + MIRROR_SHEET_ID")}
        ${statusItem("Gmail watcher (supplier invoices â rows)", status.gmail, "Set GOOGLE_SA_JSON + GMAIL_DELEGATED_USER (with domain-wide delegation)")}
      </ul>
      ${user.role === "admin" ? `
...
export function grossProfitJobPage({ user, job }) {
  ...
  const itemsHtml = (rows) => rows.length === 0 ? `<p class="muted">â</p>` : `<table class="data-table"><tbody>${rows.map(r => `<tr><td>${escape(r.description || r.labor_name || "")}</td><td>${escape(r.labor_type || "")}</td><td class="num">${money(r.amount)}</td><td class="muted">${escape(r.source || "")}</td></tr>`).join("")}</tbody></table>`;

  const att = job.attachments.map(a => `<li><a href="/gross-profit/attachment/${a.id}">${escape(a.filename)}</a> <span class="muted">${escape(a.source)}${a.supplier ? " Â· " + escape(a.supplier) : ""}</span></li>`).join("");
...
<div class="page-head">
  ...
  <span class="page-sub">Invoice ${escape(job.jobber_invoice_number || "â")} Â· ${escape(job.address || "")}${job.city ? ", " + escape(job.city) : ""} ${escape(job.zip || "")}</span>
</div>
<div class="kpi-grid">
  <div class="kpi-card"><div class="kpi-label">Amount paid</div><div class="kpi-value">${money(job.amount_paid)}</div><div class="kpi-sub">${escape(job.payment_method || "â")}</div></div>
  <div class="kpi-card"><div class="kpi-label">Equip + Materials</div><div class="kpi-value">${money(job.equipment_materials_total)}</div><div class="kpi-sub">Equip ${money(job.equipment_cost)} Â· Mat ${money(job.materials_cost)}</div></div>
  <div class="kpi-card"><div class="kpi-label">Labor + Other</div><div class="kpi-value">${money((job.total_labor_cost || 0) + (job.total_other_expenses || 0))}</div><div class="kpi-sub">Labor ${money(job.total_labor_cost)} Â· Other ${money(job.total_other_expenses)}</div></div>
...
<section class="panel">
  ...
  <table class="data-table"><tbody>
    <tr><td>Salesperson</td><td>${escape(job.salesperson_name || "â")}</td></tr>
    <tr><td>Sales commission</td><td>${money(job.sales_commission_amount)} (${pct(job.sales_commission_rate)})</td></tr>
    <tr><td>Sales manager</td><td>${escape(job.sales_manager_name || "â")} â ${money(job.sales_manager_fee)}</td></tr>
    <tr><td>Permit</td><td>${job.permit_required == null ? "â" : (job.permit_required ? "Yes" : "No")} â ${money(job.permit_fee)}</td></tr>
    <tr><td>Fee</td><td>${money(job.fee_amount)} (${escape(job.fee_type || "â")})</td></tr>
  </tbody></table>
</section>
<section class="panel">
  <h2>Attached docs</h2>
...
<p><a href="/gross-profit">â back to all jobs</a></p>
`,
  });
}
