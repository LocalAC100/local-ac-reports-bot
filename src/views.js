...
<head>
  ...
  <title>${escape(title)} ÃÂ· Control Room</title>
...
<button class="mobile-nav-toggle" onclick="document.querySelector('.primary-nav').classList.toggle('open')" aria-label="Open menu">Ã¢ÂÂ°</button>`;
}
...
<div class="kpi-grid">
  <div class="kpi-card">
    ...
    <div class="kpi-sub">${activeNow.map(escape).join(" ÃÂ· ") || "Ã¢ÂÂ"}</div>
  </div>
  <div class="kpi-card">
...
<section class="panel">
  ...
  ${discrepancies.length === 0
    ? `<div class="empty-good">Ã¢ÂÂ Everyone scheduled today is clocked in. No issues.</div>`
    : `<ul class="alert-list">${discrepancies.map(d => `<li><strong>${escape(d.employee)}</strong> Ã¢ÂÂ ${escape(d.detail)}</li>`).join("")}</ul>`}
</section>
...
<section class="panel">
  ...
  ${recentAlerts.length === 0
    ...
    : `<ul class="alert-list">${recentAlerts.map(a => `<li><span class="badge badge-red">Ã°ÂÂÂ´ ${a.minutes_elapsed}m</span> <strong>${escape(a.contact_name || "(unnamed)")}</strong> ÃÂ· ${escape(a.phone || "")} ÃÂ· ${escape(a.fired_at)}</li>`).join("")}</ul>`}
</section>`,
  });
}
...
<div class="page-head">
  <h1>Ask Claude</h1>
  <span class="page-sub">Ask anything about your business Ã¢ÂÂ Claude has access to Hubstaff, GoHighLevel, and Jobber.</span>
</div>
${!hasApiKey ? `<div class="banner-warn">Ã¢ÂÂ  Anthropic API key not configured yet. Add <code>ANTHROPIC_API_KEY</code> in Render's Environment tab to activate.</div>` : ""}
<div class="chat-wrap">
...
form?.addEventListener('submit', async (e) => {
  ...
  history.insertAdjacentHTML('beforeend', '<div class="msg msg-assistant pending"><div class="msg-role">Claude</div><div class="msg-body">Ã¢ÂÂ¦thinkingÃ¢ÂÂ¦</div></div>');
...
<section class="panel">
  <table class="data-table">
    ...
    <tbody>${users.map(u => `<tr>
      ...
      <td class="muted">${escape((u.last_login_at || "Ã¢ÂÂ").slice(0, 16))}</td>
    </tr>`).join("")}</tbody>
  </table>
</section>
<section class="panel">
...
function pct(v) {
  if (v == null || v === "") return "Ã¢ÂÂ";
  const n = parseFloat(v);
  if (!Number.isFinite(n)) return "Ã¢ÂÂ";
  return n.toFixed(1) + "%";
}
function gpClass(p) {
  if (p == null) return "";
...
export function grossProfitPage({ user, jobs, unmatched, inventory, status, flash, filter = {}, totalCount = 0, totalPaid = 0, grandTotalCount = 0, summary = null }) {
  ...
  const yesBadge = `<span class="badge badge-ok" title="Yes">✓</span>`;
  const noBadge = `<span class="badge badge-warn" title="No">✗</span>`;
  const fmtMoney = (n) =>
...
<section class="panel" style="background:#f9fafc">
  <div style="display:flex;justify-content:space-between;align-items:baseline;flex-wrap:wrap;gap:8px">
    ...
    <div class="muted" style="font-size:13px">
      ...
      â¢ Total paid: <strong>${Number(totalPaid || 0).toLocaleString(undefined, { maximumFractionDigits: 2, minimumFractionDigits: 2 })}</strong>
    </div>
  </div>
  ...
  const rows = jobs.map((j) => {
    ...
    return `<tr>
      ...
      <td>${escape(j.jobber_invoice_number || "Ã¢ÂÂ")}</td>
  ...
  const tableHtml = empty
    ? `<div class="empty-good">No jobs yet Ã¢ÂÂ waiting for first Jobber invoice. Open the <a href="/gross-profit/setup">setup checklist</a> to wire up the integrations.</div>`
    : `<table class="data-table">
        <thead><tr>
  ...
  const unmatchedHtml = unmatched.length === 0 ? "" : `
    <section class="panel panel-warn">
      <h2>Ã¢ÂÂ  Unmatched Invoices Ã¢ÂÂ Review Manually</h2>
      <table class="data-table">
        ...
        <tbody>${unmatched.map(u => `<tr>
          ...
          <td>${escape(u.po_name || "Ã¢ÂÂ")}</td>
          <td>${money(u.total_amount)}</td>
          <td>${u.att_id ? `<a href="/gross-profit/attachment/${u.att_id}">${escape(u.filename || "PDF")}</a>` : "Ã¢ÂÂ"}</td>
          <td class="muted">${escape((u.created_at || "").slice(0,16))}</td>
          <td><a href="/gross-profit/unmatched/${u.id}/resolve">Resolve Ã¢ÂÂ</a></td>
        </tr>`).join("")}</tbody>
      </table>
    </section>`;
  ...
  // Status panel Ã¢ÂÂ shows whether each connector is wired up
  const statusItem = (label, ok, hint) =>
    `<li><span class="status-dot ${ok ? "ok" : "off"}"></span> <strong>${escape(label)}</strong> Ã¢ÂÂ ${ok ? "ready" : escape(hint || "not configured")}</li>`;
  ...
  const setupHtml = `
    <section class="panel">
      ...
      <ul class="status-list">
        ${statusItem("Jobber sync (invoices Ã¢ÂÂ rows)", status.jobber, "Set JOBBER_CLIENT_ID, CLIENT_SECRET, ACCESS_TOKEN, REFRESH_TOKEN")}
        ${statusItem("Google Sheets (Chris's sheet Ã¢ÂÂ rows)", status.sheets && status.chrisSheetId, "Set GOOGLE_SA_JSON + CHRIS_SHEET_ID")}
        ${statusItem("Mirror sheet (rows Ã¢ÂÂ Google Sheet)", status.sheets && status.mirrorSheetId, "Set GOOGLE_SA_JSON + MIRROR_SHEET_ID")}
        ${statusItem("Gmail watcher (supplier invoices Ã¢ÂÂ rows)", status.gmail, "Set GOOGLE_SA_JSON + GMAIL_DELEGATED_USER (with domain-wide delegation)")}
      </ul>
      ${user.role === "admin" ? `
...
export function grossProfitJobPage({ user, job }) {
  ...
  const itemsHtml = (rows) => rows.length === 0 ? `<p class="muted">Ã¢ÂÂ</p>` : `<table class="data-table"><tbody>${rows.map(r => `<tr><td>${escape(r.description || r.labor_name || "")}</td><td>${escape(r.labor_type || "")}</td><td class="num">${money(r.amount)}</td><td class="muted">${escape(r.source || "")}</td></tr>`).join("")}</tbody></table>`;

  const att = job.attachments.map(a => `<li><a href="/gross-profit/attachment/${a.id}">${escape(a.filename)}</a> <span class="muted">${escape(a.source)}${a.supplier ? " ÃÂ· " + escape(a.supplier) : ""}</span></li>`).join("");
...
<div class="page-head">
  ...
  <span class="page-sub">Invoice ${escape(job.jobber_invoice_number || "Ã¢ÂÂ")} ÃÂ· ${escape(job.address || "")}${job.city ? ", " + escape(job.city) : ""} ${escape(job.zip || "")}</span>
</div>
<div class="kpi-grid">
  <div class="kpi-card"><div class="kpi-label">Amount paid</div><div class="kpi-value">${money(job.amount_paid)}</div><div class="kpi-sub">${escape(job.payment_method || "Ã¢ÂÂ")}</div></div>
  <div class="kpi-card"><div class="kpi-label">Equip + Materials</div><div class="kpi-value">${money(job.equipment_materials_total)}</div><div class="kpi-sub">Equip ${money(job.equipment_cost)} ÃÂ· Mat ${money(job.materials_cost)}</div></div>
  <div class="kpi-card"><div class="kpi-label">Labor + Other</div><div class="kpi-value">${money((job.total_labor_cost || 0) + (job.total_other_expenses || 0))}</div><div class="kpi-sub">Labor ${money(job.total_labor_cost)} ÃÂ· Other ${money(job.total_other_expenses)}</div></div>
...
<section class="panel">
  ...
  <table class="data-table"><tbody>
    <tr><td>Salesperson</td><td>${escape(job.salesperson_name || "Ã¢ÂÂ")}</td></tr>
    <tr><td>Sales commission</td><td>${money(job.sales_commission_amount)} (${pct(job.sales_commission_rate)})</td></tr>
    <tr><td>Sales manager</td><td>${escape(job.sales_manager_name || "Ã¢ÂÂ")} Ã¢ÂÂ ${money(job.sales_manager_fee)}</td></tr>
    <tr><td>Permit</td><td>${job.permit_required == null ? "Ã¢ÂÂ" : (job.permit_required ? "Yes" : "No")} Ã¢ÂÂ ${money(job.permit_fee)}</td></tr>
    <tr><td>Fee</td><td>${money(job.fee_amount)} (${escape(job.fee_type || "Ã¢ÂÂ")})</td></tr>
  </tbody></table>
</section>
<section class="panel">
  <h2>Attached docs</h2>
...
<p><a href="/gross-profit">Ã¢ÂÂ back to all jobs</a></p>
`,
  });
}
