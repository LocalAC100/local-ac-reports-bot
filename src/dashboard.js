// Dashboard router ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ login, all pages, /api/ask endpoint, /logout.
...
function fmtET(iso) {
  if (!iso) return "ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ";
...
// Filter for installs only Ã¢ÂÂ excludes $0 invoices (warranty/recall/service)
...
export function buildDashboardRouter() {
  ...
  router.get("/", async (req, res) => {
    ...
    // Best-effort live data ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ failures don't break the page
    try {
    ...
    try {
      ...
      const rows = EMPLOYEES.filter((e) => e.hubstaffEmail).map((e) => {
        ...
        return `<tr>
          ...
          <td>${hu ? "ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ linked" : "<span class='badge badge-amber'>not in Hubstaff</span>"}</td>
          <td class="muted">${hu?.last_activity ? new Date(hu.last_activity).toLocaleString("en-US", { timeZone: "America/New_York" }) : "ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ"}</td>
        </tr>`;
      }).join("");
      ...
      const rows = dispatchers
        .map((e) => {
          ...
          return `<tr>
            ...
            <td>${matched ? "ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ linked" : "<span class='badge badge-amber'>not in GHL</span>"}</td>
            <td>${e.role}</td>
          </tr>`;
        })
        .join("");
  ...
  router.get("/leads", async (req, res) => {
    res.send(
      views.placeholderPage({
        ...
        title: "Leads",
        navKey: "leads",
        body: `<p class="muted">Recent leads from GoHighLevel will show here. Live alerts fire when a lead isn't contacted within 3 minutes ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ see the Alerts tab for that history.</p>`,
      })
    );
  });
      ...
      alerts.length === 0
        ...
        : `<table class="data-table">
            ...
            <tbody>${alerts
              ...
              <td>${a.phone || "ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ"}</td>
  ...
  router.get("/reports/:id", (req, res) => {
    ...
    // Reports are stored as raw HTML ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ wrap in our layout
    res.send(
      views.placeholderPage({
        user: req.user,
  ...
  router.get("/gross-profit", (req, res) => {
    ...
    if (!from && !to) {
      ...
      // preset === "all" ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ leave from/to null
    }
  ...
  // Manual sync triggers (admin only) ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ useful before crons fire
  ...
  router.get("/gross-profit/debug/reconcile", requireAdmin, (req, res) => {
    ...
    res.json({ summary, partialPaid: partial, noInvoiceTotal: noTotal });
  });
